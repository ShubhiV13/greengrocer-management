...
# ===================== AUTO-CREATE PRODUCTS FUNCTION =====================
def create_default_products():
    """Create default products if database is empty"""
    print("🔍 Checking if products need to be created...")
    
    count = Product.query.count()
    print(f"📊 Current product count: {count}")
    
    if count == 0:
        print("🛒 No products found. Creating default products...")
        ...
        db.session.commit()
        print(f"✅ SUCCESS: Auto-created {len(sample_products)} default products")
...
def pos():
    ...
    # AUTO-CREATE PRODUCTS IF NONE EXIST (BACKUP)
    if Product.query.count() == 0:
        print("⚠️ No products found in POS route - creating them now")
        create_default_products()
        flash('Products have been added automatically!', 'success')
...
if __name__ == '__main__':
    with app.app_context():
        ...
        create_default_products()  # 👈 THIS AUTO-CREATES PRODUCTS!
